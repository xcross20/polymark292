...
COINBASE_SYMBOLS = {"BTC": "BTC-USD", "ETH": "ETH-USD", "SOL": "SOL-USD"}


def get_coinbase_momentum(symbol="BTC-USD", lookback_minutes=5):
    """Get price momentum from Coinbase Exchange public API (no API key needed).
    Returns: {momentum_pct, direction, price_now, price_then, avg_volume, candles}
    """
    url = (
...
def get_momentum(asset="BTC", source="coinbase", lookback=5):
    ...
    if source == "coinbase":
        symbol = COINBASE_SYMBOLS.get(asset, "BTC-USD")
...
def run_fast_market_strategy(dry_run=True, positions_only=False, show_config=False,
    ...
    log("[BOLT] Simmer FastLoop Trading Skill")
    log("=" * 50)
    ...
    log(f"  Asset:            {ASSET}")
    log(f"  Window:           {WINDOW}")
    log(f"  Entry threshold:  {ENTRY_THRESHOLD} (min divergence from 50c)")
    ...
    log(f"  Volume weighting: {'Y' if VOLUME_CONFIDENCE else 'N'}")
    ...
    if not markets:
        ...
        if not quiet:
            print("[STATS] Summary: No markets available")
        return
    ...
    if not best:
        ...
        if not quiet:
            print("[STATS] Summary: No tradeable fast_markets (too close to expiry)")
        return
    ...
    log(f"\n[CHART] Fetching {ASSET} price signal ({SIGNAL_SOURCE})...")
    ...
    if momentum_pct < MIN_MOMENTUM_PCT:
        log(f"  [PAUSE]  Momentum {momentum_pct:.3f}% < minimum {MIN_MOMENTUM_PCT}% -- skip")
        if not quiet:
            print(f"[STATS] Summary: No trade (momentum too weak: {momentum_pct:.3f}%)")
        return
    ...
    if VOLUME_CONFIDENCE and momentum["volume_ratio"] < 0.5:
        log(f"  [PAUSE]  Low volume ({momentum['volume_ratio']:.2f}x avg) -- weak signal, skip")
        if not quiet:
            print(f"[STATS] Summary: No trade (low volume)")
        return
    elif VOLUME_CONFIDENCE and momentum["volume_ratio"] > 2.0:
        vol_note = f" [STATS] (high volume: {momentum['volume_ratio']:.1f}x avg)"

    # Check divergence threshold
    if divergence <= 0:
        log(f"  [PAUSE]  Market already priced in: divergence {divergence:.3f} <= 0 -- skip")
        if not quiet:
            print(f"[STATS] Summary: No trade (market already priced in)")
        return
    ...
    if fee_rate > 0:
        ...
        if divergence < min_divergence:
            log(f"  [PAUSE]  Divergence {divergence:.3f} < fee-adjusted minimum {min_divergence:.3f} -- skip")
            if not quiet:
                print(f"[STATS] Summary: No trade (fees eat the edge)")
            return
    ...
    if price > 0:
        ...
        if min_cost > position_size:
            log(f"  [WARN]  Position ${position_size:.2f} too small for {MIN_SHARES_PER_ORDER} shares at ${price:.2f}")
            return

    log(f"  [OK] Signal: {side.upper()} -- {trade_rationale}{vol_note}", force=True)
    log(f"  Divergence: {divergence:.3f}", force=True)

    # Step 5: Import & Trade
    log(f"\n[LINK] Importing to Simmer...", force=True)
    ...
    log(f"  [OK] Market ID: {market_id[:16]}...", force=True)
    ...
    if dry_run:
        ...
        if result and result.get("success"):
            ...
            log(f"  [OK] Bought {shares:.1f} {side.upper()} shares @ ${price:.3f}", force=True)
